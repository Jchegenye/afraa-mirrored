    ...
    for (i = 0; i < countryInput.length; i++) {
        ...
        if (flag) {
            ...
            $.each(countries, function (index, country) {
                ...
                var flags = country.code;
                var myFlg = flags.toLowerCase();

                var flagIcon = "/plugin/bootstrap-country-picker-jQuery/css/flags/" + myFlg + ".png";
                var myflagIcon = flagIcon.replace(/&quot;/g,'/');

                countryList += "<option data-country-code='" + country.code + "' data-tokens='" + country.code + " " + country.name + "' style='padding-left:25px; background-position: 4px 7px; background-image:url("+ myflagIcon +");background-repeat:no-repeat;'value='" + country.name + "'>" + country.name + "</option>";
            });
            ...
            countryInput.eq(i).on('loaded.bs.select', function (e) {
                ...
                button.css("background-image", "url('" + myflagIcon + "'");
                button.show();
            });
